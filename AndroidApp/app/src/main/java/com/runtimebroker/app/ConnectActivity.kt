...
class ConnectActivity : BaseActivity() {
    ...
    companion object {
        private const val DEFAULT_REMOTE_SERVER = "https://windows-remote-toolkit.onrender.com"
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val cfg = Prefs.serverUrl(this).takeIf { Prefs.serverMode(this) == "connect" && it.isNotBlank() }
            ?: DEFAULT_REMOTE_SERVER
        binding.inputIp.setText(cfg)
    ...
    private fun attemptConnection() {
        ...
        val url = normalizeServerUrl(ip, port)
    ...
    private fun normalizeServerUrl(value: String, port: String): String {
        val raw = value.trim().trimEnd('/')
        if (raw.startsWith("http://") || raw.startsWith("https://")) return raw
        val isLocalAddress = raw == "localhost" || raw == "127.0.0.1" ||
            raw.matches(Regex("^10\\..+")) || raw.matches(Regex("^192\\.168\\..+")) ||
            raw.matches(Regex("^172\\.(1[6-9]|2\\d|3[01])\\..+"))
        return if (isLocalAddress) "http://$raw:$port" else "https://$raw"
    }
